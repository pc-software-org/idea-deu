...
class ProductConfigTest(unittest.TestCase):
    ...
    def test_loads_exact_product_binding(self) -> None:
        ...
        self.assertEqual(config.version, "2026.2.2")
        self.assertEqual(config.build_number, "262.10315.125")
    ...
    def test_rejects_wrong_build_number(self) -> None:
        with self.assertRaisesRegex(
            ValueError, r"build_number must be exactly 262\.10315\.125"
        ):
            self._load(self.VALID_CONFIG | {"build_number": "262.10315.338"})
    ...
    def test_accepts_widened_compatibility_bounds(self) -> None:
        for field in ("since_build", "until_build"):
            for value in ("262", "262.*", "262.10315", "262.10315.125"):
    ...
    def test_build_number_must_stay_exact(self) -> None:
        with self.assertRaisesRegex(ValueError, r"build_number must be exactly 262\.10315\.125"):
